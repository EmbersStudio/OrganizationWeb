...
const DEFAULT_BORDERS: BoxBorders = { left: 0, top: 0, right: 0, bottom: 0 };
 ...
 */
function resolveInnerRadius(borderRadius: string, outerWidth: number, outerHeight: number, border: BoxBorders): number {
  ...
  const maxBorder = Math.max(border.left, border.top, border.right, border.bottom);
 ...
 * - 指针事件挂在真实 <img> 上（效果层 pointer-events: none，点击/交互不受影响）；
 * - 每帧跟随图片几何位置（滚动、响应式尺寸变化、transform 均自适应）。
 */
export function useDecryptRevealContent(props: DecryptRevealRuntimeProps): UseDecryptRevealContentResult {
  ...
  useEffect(() => {
    ...
    const applyCanvasGeometry = (entry: ImgEntry, rect: DOMRect): boolean => {
      const cssW = Math.max(0, rect.width - entry.borders.left - entry.borders.right);
      const cssH = Math.max(0, rect.height - entry.borders.top - entry.borders.bottom);
      if (cssW < 1 || cssH < 1) {
        if (entry.canvas) {
      ...
      const radius = resolveInnerRadius(entry.borderRadiusRaw, rect.width, rect.height, entry.borders);
    ...
    const syncEntry = (entry: ImgEntry): void => {
      ...
      const sizeChanged =
        Math.abs(cssW - entry.lastCssW) > 0.5 || Math.abs(cssH - entry.lastCssH) > 0.5 || dpr !== entry.lastDpr;
      ...
      // 内容栅格：仅在尺寸 / 样式 / 图片变化时重绘（复刻浏览器 object-fit 渲染）
      ...
      if (needRaster) {
        ...
        if (ctx) {
          ...
          try {
            ...
            console.error('[DecryptReveal] 无法读取图片像素（CORS 限制？）:', img.src, error);
            entry.rasterDirty = false;
            if (entry.canvas) {
              entry.canvas.style.display = 'none';
            }
      ...
      if (!entry.renderer) {
        try {
          ...
          if (entry.lastPointer?.active === 1) {
            renderer.setPointerTarget(entry.lastPointer.x, entry.lastPointer.y, 1);
          }
        } catch (error) {
          console.error('[DecryptReveal] 图片效果初始化失败（已回退为普通图片）:', img.src, error);
          if (entry.canvas) {
            entry.canvas.remove();
            entry.canvas = null;
          }
    ...
    const onImgPointerLeave = (event: PointerEvent): void => {
      ...
      entry.lastPointer = { ...(entry.lastPointer ?? { x: -100000, y: -100000 }), active: 0 };
    ...
    observer.observe(wrapper, {
      ...
      childList: true,
      attributes: true,
      attributeFilter: ['src', 'srcset', 'class', 'style', 'crossorigin', 'alt', 'width', 'height'],
    });
